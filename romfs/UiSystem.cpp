...
void UiSystem::configure(ECS::World* world)
{
    ...
    mStatusMessage = mLanguageFile.get("ready");
}
    ...
    if (mShowWorkSpace)
    {
                ...
                for (auto row=clipper.DisplayStart; row<clipper.DisplayEnd; ++row)
                {
                    ...
                    if (ImGui::Selectable(rowId, rowIsSelected, ImGuiSelectableFlags_SpanAllColumns))
                    {
                        ...
                        else
                        {
                            ...
                            auto message = fmt::format("{:s} {:s}", mLanguageFile.get("unsupported_file_type"), item.name);
                    ...
                    {
                        auto menuItemId = fmt::format("\uf416 {:s}", mLanguageFile.get("add_to_playlist"));
        ...
        if (ImGui::BeginChild("##playerWrapper", windowSize, false, windowFlags))
        {
            ...
            else
            {
                ...
                {
                    Plugin::drawRow(mLanguageFile.get("player.title"),      "");
                    Plugin::drawRow(mLanguageFile.get("player.track"),      "");
                    Plugin::drawRow(mLanguageFile.get("player.duration"),   "");
                    Plugin::drawRow(mLanguageFile.get("player.position"),   "");
                    Plugin::endTable();
                }
        ...
        else
        {
            ...
            else
            {
                ...
                auto fileStr = mLanguageFile.get("file_s");
...
void UiSystem::receive(ECS::World* world, const DirectoryLoadedEvent& event)
{
    ...
    if (mLoadDirectoryParams.addToPlaylist)
    {
        ...
        auto itemsAddedStr = fmt::format("{:d} {:s}", itemsAdded, mLanguageFile.get("files_added_to_playlist"));
...
void UiSystem::receive(ECS::World* world, const AudioSystemPlayEvent& event)
{
    ...
    switch (event.type)
    {
        ...
        case AudioSystemPlayEvent::STOPPED_BY_USER:
                mAudioSystemStatus = STOPPED;
                mStatusMessage = mLanguageFile.getc("ready");
                resetPlaylist(false);
                mCurrentPluginUsed.reset();
        break;
...
void UiSystem::processNextPlaylistItem(ECS::World* world)
{
    ...
    {
        pushNotification(Notification::INFO, mLanguageFile.get("playlist_finished"));
