...
It links `Cursor`, `PromptCursor`, `UndoHistory`, `LineBuffer`, `LongestLineTracker`, `CommandLine`, `OskLayout`, `KeyModifiers`, `LineScanner`, `PromptState`/`ViewState` and the four CVar types only — it compiles against the SDL headers but links no SDL, GL, FreeType or tree-sitter library — so it builds in seconds. doctest is vendored at `tests/doctest.h` (MIT, never edited); nothing comes from vcpkg. `tests/` is outside the toolchain's reach: `misc/pre-commit` globs `src/*.cpp` and `src/*.h`, and `.clang-tidy` filters headers to `.*/src/.*`, so nothing there is linted.
...
- `OskLayoutTests.cpp` — the nine OSK layouts through `findLayout`/`resolve`: the US base, the letter permutations, the accents keyed by the displayed letter (not the slot), sweeps over all layouts × scancodes for the never-blank and digits-stay-digits invariants, and the accent-reachability check (every accent's letter is displayed by some slot). The failure mode it exists for: one wrong character on one layout that nobody on a qwerty desktop would ever see.
- `KeyModifiersTests.cpp` — the left/right folding, the pad bits passing through, `fromName`/`forEachName` agreeing with the map, and above all Caps Lock/Num Lock being dropped so bindings still match with a lock on.
- `LineScannerTests.cpp` — `LineScanner`'s matching against a per-position scan that folds at comparison time, and its self-overlap answer against building a witness string carrying two overlapping occurrences, so the two derivations cannot be wrong together; the high-value target is `isSelfOverlapping` under case folding, which backward match-stepping relies on.
- `LineEndingTests.cpp` — `detectLineEnding` and `applyLineEnding`, the CRLF convention `OpenFileCommand::readFile` counts votes for and `SaveFileCommand` writes back. A statement-for-statement simulation of readFile's getline/pop/count loop runs against a raw-byte majority count over every string up to length 7 on `{a, \n, \r}`, so the loop-shaped and byte-shaped derivations cannot be wrong together; round-trip cases prove a purely-CRLF file comes back byte-exact, and named edges pin the strict majority (a tie stays LF) and the stray EOF `\r` that must not vote. Also `Cursor`'s per-buffer convention: default LF, untouched by `loadContent`.
- `OpenSizeLimitTests.cpp` — `exceedsOpenSizeLimit`, the threshold behind the open command's large-file confirmation: a zero or negative limit never fires, the boundary at exactly `limit * 1024 * 1024` bytes is exclusive, a megabyte is 1024×1024, and the widest limit does not overflow.
...
**Platform seam** (src/platform/Platform.h): the few desktop/Switch behavior differences live behind static methods of the `Platform` class, with one CMake-selected implementation — `src/platform/PlatformDesktop.cpp` or `src/platform/PlatformSwitch.cpp` (the only file allowed to include `<switch.h>`). The `src/platform/` directory also holds the Switch SDL2 patch. `assetPath()` resolves the `romfs/` prefix (`romfs:/` on Switch), used by `ApplicationWindow::create` and `ExecCommand` so scripts say `exec romfs/...` everywhere; `userConfigDir()` returns the directory holding the NRO on Switch, where `ApplicationWindow::create` seeds and then runs a writable copy of `autoexec` (the packaged one being read-only), and `nullopt` on desktop — the seeding copies through plain streams because `std::filesystem::copy_file` calls `fchmod`, which libnx's SD card device answers with `ENOSYS`, so any file operation added for Switch has to stay on open/read/write; `preferredColorScheme()` returns the console color set on Switch (applied as light/dark theme before autoexec, so scripted colors win) and `nullopt` on desktop; `keyboardLayout()` returns the console keyboard layout as an OSK layout name on Switch (falling back to `"qwerty"` for layouts without a table) and `"qwerty"` on desktop, giving the OSK its initial layout; `addControllerMappings()` overrides the console pad's positional SDL mapping with a label-true one on Switch (so `pad:a` is the labeled A) and is a no-op on desktop.
